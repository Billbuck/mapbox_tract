...
// LOG DE DEBUG
console.log('[ENTETE] Chargement du module entête');
...
function loadScript(scriptName, callback) {
    ...
    script.onload = function() {
        console.log('[ENTETE] ✅ Script chargé: ' + scriptName);
        callback();
    };
...
function loadCSS(cssName, callback) {
    ...
    link.onload = function() {
        console.log('[ENTETE] ✅ CSS chargé: ' + cssName);
        callback();
    };
...
function loadExternalCSS(url, callback) {
    ...
    link.onload = function() {
        console.log('[ENTETE] ✅ CSS externe chargé: ' + url.split('/').pop());
        callback();
    };
...
function loadExternalScript(url, callback) {
    ...
    script.onload = function() {
        console.log('[ENTETE] ✅ Bibliothèque chargée: ' + url.split('/').pop());
        callback();
    };
    ...
    for (var i = 0; i < externalCSS.length; i++) {
        loadExternalCSS(externalCSS[i], function() {
            ...
            if (count === externalCSS.length) {
                console.log('[ENTETE] ✅ Tous les CSS externes sont chargés');
                callback();
            }
        });
    }
...
function loadAllExternalLibraries(callback) {
    ...
    function loadNextLibrary() {
        if (currentIndex >= externalLibraries.length) {
            console.log('[ENTETE] ✅ Toutes les bibliothèques externes sont chargées');
            callback();
            return;
        }
    ...
    for (var i = 0; i < cssToLoad.length; i++) {
        loadCSS(cssToLoad[i], function() {
            ...
            if (cssCount === cssToLoad.length) {
                console.log('[ENTETE] ✅ Tous les CSS locaux sont chargés');
                callback();
            }
        });
    }
...
function loadNextScript() {
    if (loadedScripts >= scriptsToLoad.length) {
        console.log('[ENTETE] ✅ Tous les scripts sont chargés');
...
function onAllScriptsLoaded() {
    ...
    console.log('[ENTETE] 🚀 Module France prêt pour l\'initialisation');
...
function InitialiserModuleFrance() {
    console.log('[ENTETE] === InitialiserModuleFrance ===');
    
    if (FRANCE_MODULE_STATE.isInitialized) {
        console.log('[ENTETE] Module déjà initialisé');
        return;
    }
    
    if (!FRANCE_MODULE_STATE.scriptsLoaded) {
        console.log('[ENTETE] Scripts pas encore chargés, nouvelle tentative dans 500ms...');
    ...
    if (window.CommunicationCarteWebDev) {
        ...
        try {
            ...
            console.warn('[ENTETE] Impossible de récupérer les alias:', e);
        }
    ...
    if (window.initApp) {
        ...
        waitForMap(function() {
            ...
            console.log('[ENTETE] ✅ Module France initialisé avec succès');
        });
    } else {
...
function lireVariablesWebDev() {
    console.log('[ENTETE] Lecture des variables WebDev...');
    ...
    if (window.WebDevBridge) {
        ...
        if (zonesJson) {
            try {
                ...
                console.log('[ENTETE] Zones sélectionnées lues:', FRANCE_MODULE_STATE.webdevVariables.zonesSelectionnees);
            } catch (e) {
                console.warn('[ENTETE] Impossible de parser sZonesSelectionnees:', e);
            }
        }
        ...
        console.log('[ENTETE] Variables lues avec WebDevBridge:', FRANCE_MODULE_STATE.webdevVariables);
        ...
        try {
            ...
            console.log('[ENTETE] Variables lues en accès direct:', FRANCE_MODULE_STATE.webdevVariables);
...
function gererEtatInitial() {
    console.log('[ENTETE] === gererEtatInitial() - VERSION REFONTE ===');
    ...
    if (window.WebDevBridge) {
        ...
        if (zonesJson) {
            try {
                ...
                console.log('[ENTETE] Données de zones récupérées:', zonesData);
            } catch (e) {
                console.warn('[ENTETE] Impossible de parser sZonesSelectionnees:', e);
            }
        }
    ...
    if (!vars.latitude || !vars.longitude || !vars.adresse) {
        console.log('[ENTETE] Mode création - Aucune adresse détectée');
        console.log('[ENTETE] Variables actuelles:', {
        ...
        console.log('[ENTETE] Interface masquée - En attente de validation adresse');
        ...
        setTimeout(function() {
            if (window.openAddressPopup) {
                console.log('[ENTETE] Ouverture popup adresse');
    ...
    if (zonesData && zonesData.codes && zonesData.codes.length > 0) {
        console.log('[ENTETE] Mode restauration - Zones existantes détectées');
        ...
        if (zonesData.type_zone) {
            ...
            if (selector) {
                ...
                console.log('[ENTETE] Type de zone restauré EN PREMIER:', zonesData.type_zone);
            }
            ...
            if (window.updateTypeZoneWebDev) {
                ...
                console.log('[ENTETE] Type de zone communiqué à WebDev:', zonesData.type_zone);
            }
        }
        ...
        console.log('[ENTETE] Ajout de interface-visible (mode restauration)');
        ...
        if (zonesData.adresse && zonesData.latitude && zonesData.longitude) {
            ...
            if (zonesData.adresse !== vars.adresse || 
                ...
                console.log('[ENTETE] Adresse modifiée détectée, utilisation de l\'adresse de la sélection');
        ...
        // Charger les zones sélectionnées après un délai
        setTimeout(function() {
            console.log('[ENTETE] Chargement des zones sélectionnées:', zonesData.codes.length);
    ...
    // CAS 3 : Première visite avec adresse pré-remplie (mode Modification)
        ...
        console.log('[ENTETE] Mode modification - Adresse initiale présente');
        console.log('[ENTETE] Adresse:', vars.adresse);
        console.log('[ENTETE] Coordonnées:', { lat: vars.latitude, lng: vars.longitude });
        ...
        if (zonesData && zonesData.type_zone) {
            ...
            if (selector) {
                selector.value = zonesData.type_zone;
                console.log('[ENTETE] Type de zone initialisé:', zonesData.type_zone);
            }
        ...
        // NOUVEAU : Afficher l'interface car on a une adresse valide
        console.log('[ENTETE] Ajout de interface-visible (adresse pré-remplie)');
        document.body.classList.add('interface-visible');
        ...
        if (window.WebDevBridge) {
            ...
            console.log('[ENTETE] Initialisation sZonesSelectionnees:', initialData);
        }
    }
    
    console.log('[ENTETE] === Fin gererEtatInitial() ===');
}
...
function chargerZonesSelectionnees(zonesData) {
    ...
    console.log('[ENTETE] Chargement des zones:', zonesData);
...
window.RecupererDonneesSelection = function() {
    ...
    if (window.WebDevBridge) {
        var zonesJson = WebDevBridge.get('sZonesSelectionnees');
        console.log('[ENTETE] RecupererDonneesSelection - Données actuelles:', zonesJson);
        return zonesJson;
    }
...
window.ReinitialiserSelection = function() {
    console.log('[ENTETE] === ReinitialiserSelection ===');
    ...
    if (window.WebDevBridge) {
        ...
        console.log('[ENTETE] sZonesSelectionnees réinitialisé avec WebDevBridge');
    }
};
...
console.log('[ENTETE] 📦 Début du chargement des ressources Module France...');
...
console.log('[ENTETE] ✅ ENTETE-FRANCE.JS chargé - En attente du chargement des ressources...');